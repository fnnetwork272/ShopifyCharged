    ...
    'operationName': 'SubmitForCompletion',
        ...
        try:
            async with r.post(
                ...
                try:
                    res_json = json.loads(text)
                    receipt = res_json.get('data', {}).get('receipt', {})
                    
                    # Extract status based on receipt type
...
[⌬]𝗣𝗿𝗼𝘅𝘆 -» {proxy_status}""", 
                        ...
                        error_type = error_info.get('__typename', '')
                        error_code = error_info.get('code', '')
                        error_msg = error_info.get('messageUntranslated', '')
                        
                        # Format response message
                        response_msg = error_type
                        if error_code:
                            response_msg += f" - {error_code}"
                        if error_msg:
                            response_msg += f": {error_msg}"
...
[ϟ]𝗖𝗮𝗿𝗱 -» {full_card}
[ϟ]𝗚𝗮𝘁𝗲𝘄𝗮𝘆 -» Shopify 14$
[ϟ]𝗥𝗲𝘀𝗽𝗼𝗻𝘀𝗲 -» Unexpected response
...
[⌬]𝗣𝗿𝗼𝘅𝘆 -» {proxy_status}""", 
                        ...
                        fff = find_between(text, '"code":"', '"') or find_between(text, '"message":"', '"')
                        return ShResult(
                            f"""𝐃𝐞𝐜𝐥𝐢𝐧𝐞𝐝 ❌

[ϟ]𝗖𝗮𝗿𝗱 -» {full_card}
[ϟ]𝗚𝗮𝘁𝗲𝘄𝗮𝘆 -» Shopify 14$
[ϟ]𝗥𝗲𝘀𝗽𝗼𝗻𝘀𝗲 -» {fff or "Declined"}
