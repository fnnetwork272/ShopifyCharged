    ...
    while retry_count < max_retries:
        try:
            async with r.post(
                ...
                try:
                    response_json = json.loads(final_response_text)
...
"""
                        ...
                        error_message = (
                            processing_error.get('messageUntranslated') or
                            processing_error.get('code') or
                            processing_error.get('message') or
                            "Generic Decline"
                        )
                        
                        # Special handling for payment failed errors
...
"""
                    ...
                    if "thank" in final_response_text.lower():
                        result = f"""Card: {full_card}
Status: Charged🔥
Response: Order # confirmed
...
"""
                        log_event("INFO", f"Card charged: {full_card}", None)
                        return result, elapsed_time, proxy_status
                    elif "actionrequiredreceipt" in final_response_text.lower():
                        result = f"""Card: {full_card}
Status: Approved!✅
Response: ActionRequired
...
"""
                        log_event("INFO", f"Card approved: {full_card}", None)
                        return result, elapsed_time, proxy_status
                    elif "processingreceipt" in final_response_text.lower():
                        ...
                        patterns = [
                            ...
                            r'"error":"(.*?)"',
                            r'"reason":"(.*?)"',
                            r'payment_failed.*?code":"(.*?)"',
                            r'"error_message":"(.*?)"'
                        ]
                        
                        for pattern in patterns:
                            match = re.search(pattern, final_response_text)
