    ...
    async with aiohttp.ClientSession(connector=connector) as r:
        ...
        try:
            async with r.post(
                ...
                # Use your proven response extraction method
                fff = find_between(text, '"code":"', '"')
                
                # Fallback to other patterns if needed
                if fff == "Unknown":
                    # Try different patterns to find the error message
                    patterns = [
                        r'"message":"(.*?)"',
                        r'"decline_reason":"(.*?)"',
                        r'"error":"(.*?)"',
                ...
                # Determine card status based on response content
...
"""
                    ...
                    # Handle processing receipt with a delay
...
Bot by: ElectraOp
"""
                    log_event("INFO", f"Card processing: {full_card}", None)
                else:
                    result = f"""Card: {full_card}
Status: Declined!❌
Response: {fff}
