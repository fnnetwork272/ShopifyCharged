...
async def sh(card_input: str):
    ...
    async with aiohttp.ClientSession(connector=connector) as r:
        ...
        # ... [previous code up to SubmitForCompletion remains unchanged] ...
...
try:
    ...
    proxy_manager.mark_bad(proxy)
    proxy_status = "Dead"
    log_event("ERROR", "SubmitForCompletion POST timed out", None)
    return "SubmitForCompletion request timed out", time.time()-start_time, proxy_status
except Exception as e:
    log_event("ERROR", f"SubmitForCompletion POST exception: {str(e)}", None)
    return "SubmitForCompletion request failed", time.time()-start_time, proxy_status

# Define headers, params, and json_data for PollForReceipt
headers = {
    'authority': 'www.buildingnewfoundations.com',
    'accept': 'application/json',
    'accept-language': 'en-US',
...
for attempt in range(max_retries):
    try:
        async with r.post(
            ...
            text = await response.text()
            log_event("DEBUG", f"PollForReceipt response (attempt {attempt + 1}): {text[:500]}", None)
            
            try:
                response_json = json.loads(text)
                receipt = response_json.get('data', {}).get('receipt', {})
...
"""
                    ...
                    error_message = None
                    if processing_error.get('__typename') == 'PaymentFailed':
                        error_code = processing_error.get('code', '')
                        error_msg = processing_error.get('messageUntranslated', '') or processing_error.get('message', '')
                    ...
                    if not error_message:
                        error_message = extract_error_from_text(text)
                    if not error_message:
...
Bot by: ElectraOp
"""
                    log_event("INFO", f"Card declined: {full_card} - {error_message}", None)
                    return result, elapsed_time, proxy_status
                
                elif "processingreceipt" in receipt_type or "waitingreceipt" in receipt_type:
...
"""
                    ...
                    error_message = "Unknown receipt type"
                    elapsed_time = time.time() - start_time
                    result = f"""Card: {full_card}
Status: Declined!❌
Response: {error_message}
...
"""
                    ...
                    error_message = extract_error_from_text(text) or "Unknown Error"
                    elapsed_time = time.time() - start_time
                    result = f"""Card: {full_card}
Status: Declined!❌
Response: {error_message}
...
"""
        ...
        if attempt < max_retries - 1:
            ...
            proxy_manager.mark_bad(proxy)
            proxy_status = "Dead"
            log_event("ERROR", "PollForReceipt timed out after retries", None)
            elapsed_time = time.time() - start_time
            return "PollForReceipt request timed out after retries", elapsed_time, proxy_status
    except Exception as e:
        log_event("ERROR", f"PollForReceipt exception: {str(e)}", None)
        elapsed_time = time.time() - start_time
        return "PollForReceipt request failed", elapsed_time, proxy_status
...
Bot by: ElectraOp
"""
log_event("INFO", f"Card declined after retries: {full_card}", None)
return result, elapsed_time, proxy_status
