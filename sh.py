        ...
        json_data = {
            ...
            'variables': {
                        ...
                        'deliveryLines': [
                            {
                                ...
                                'selectedDeliveryStrategy': {
                                    'deliveryStrategyByHandle': {
                                        'handle': '94d66aff305892f7ee07ff8486315cae-89a1aeb7b7dd8e997909f14dd193e44b',
                ...
                'analytics': {
                    ...
                    'pageId': '49ba70e7-3E5F-4DEF-4112-D8A8F2010766',
                },
            },
            'operationName': 'SubmitForCompletion',
        }
...
async def multi_check(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # Parse cards
    card_lines = " ".join(context.args).split("\n")
    if len(card_lines) > 5:
        card_lines = card_lines[:5]
        await update.message.reply_text("⚠️ Only checking first 5 cards")
    
    cards = [line.strip() for line in card_lines if line.strip()]
    ...
    async with lock:
        ...
        for card, res in results:
            status = "Charged 🔥" if res.status == "Charged" else "Approved ✅" if res.status == "Approved" else "Declined ❌"
            result_text += (
                f"[ϟ]𝗖𝗮𝗿𝗱 -» <code>{card}</code>\n"
                f"[ϟ]𝗦𝘁𝗮𝘁𝘂𝘀 -» {status}\n"
                f"[ϟ]𝗚𝗮𝘁𝗲𝘄𝗮𝘆 -» Shopify 1$\n"
                f"[ϟ]𝗥𝗲𝘀𝗽𝗼𝗻𝘀𝗲 -» {res.response}\n"
                f"━━━━━━━━━━━━━━━━━━━━\n\n"
            )
