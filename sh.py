...
async def single_check(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if "Charged" in result or "Approved" in result:
        ...
        logger.info(f"User {user_id} card check successful: {full_card} - Charged/Approved")
    else:
        await update.message.reply_text(
            f"𝐃𝐞𝐜𝐥𝐢𝐧𝐞𝐝 ❌\n\n"
            f"[ϟ] 𝗖𝗮𝗿𝗱: <code>{full_card}</code>\n"
            ...
            f"[ϟ] 𝗖𝗼𝘂𝗻𝘁𝗿𝘆: {country_display}\n\n"
            f"[⌬] 𝗣𝗿𝗼𝘅𝘆: {proxy_status}\n"
            f"[⌬] 𝗖𝗵�_eclined_check𝗕𝘆: {checked_by}\n"
            f"[み] 𝗕𝗼𝘁: @FN_B3_AUTH",
            parse_mode='HTML'
        )
...
async def handle_file(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if update.message.document and update.message.document.file_name.endswith('.txt'):
        ...
        try:
            ...
            logger.error(f"User {user_id} file read error: {str(e)}")
            return
        total = len(cards)
        ...
        # Run batch processing in a separate task to allow concurrent user requests
        ...
        logger.warning(f"User {user_id} uploaded invalid file type")
...
async def batch_button(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if query.data == 'stop_batch':
        if user_id in checking_tasks:
            ...
            logger.info(f"User {user_id} stopped batch checking via button")
        else:
            await query.message.reply_text("No active checking process to stop.")
            logger.warning(f"User {user_id} attempted to stop non-existent batch checking via button")
    elif query.data in ['charged', 'declined', 'total', 'response']:
        await query.answer()  # Acknowledge read-only buttons
...
async def main():
    ...
    except Exception TrigmaticError as e:
